(() => {
  ...
  uploadFiles.onclick = () => {
    if (filesInput)
      document.body.removeChild(filesInput)
  ...
  async function finishLoadingCodeWithEmbeddedSourceMap(code, file) {
    ...
    if (!match)
      match = /\/(\*)[#@] *sourceMappingURL=((?:[^\s*]|\*[^/])+)(?:[^*]|\*[^/])*\*\//.exec(code)
  ...
  function decodeMappings(mappings, sourcesCount, namesCount) {
    ...
    while (i < n) {
      ...
      if (generatedColumnDelta < 0)
        needToSortGeneratedColumns = true
  ...
  function parseSourceMap(json) {
    ...
    if (!(Array.isArray(json.sources)) || json.sources.some(x => typeof x !== 'string')) {
  ...
  window.finishLoading = function finishLoading(code, map) {
    ...
    window.generatedTextArea = generatedTextArea = createTextArea({
      sourceIndex: null,
      text: code,
      mappings: sm.data,
      mappingsOffset: 0,
      ...
      if (devicePixelRatio !== ratio) {
        ...
        for (const contour of patternContours) {
          for (let i = 0; i < contour.length; i += 2) {
            if (i === 0)
              patternContext.moveTo(contour[i], contour[i + 1])
  ...
  function createTextArea({ sourceIndex, text, mappings, mappingsOffset, otherSource, originalName, bounds }) {
    ...
    function analyzeLine(row, column, fractionalColumn, tabStopBehavior) {
      ...
      let firstRun = 0
      let nearbyRun = 0
      const runs = row < lines.length ? lines[row].runs : emptyArray
      ...
      function rangeOfMapping(map) {
        if (mappings[map + mappingsOffset] !== row)
          return null
        const startIndex = mappings[map + mappingsOffset + 1]
        let endIndex
    ...
    return {
      ...
      draw(bodyStyle) {
        ...
        for (let row = firstRow; row <= lastRow; row++) {
          ...
          const { firstRun, runs, firstMapping, endOfLineColumn, rangeOfMapping, columnToIndex } = analyzeLine(
            row,
            firstColumn,
            firstColumn,
            'floor',
        ...
        if (scrollbarY) {
          const dx = x + width - scrollbarThickness
  ...
  canvas.addEventListener(
    ...
    (e) => {
      e.preventDefault()
      if (originalTextArea)
        originalTextArea.onwheel(e)
      if (generatedTextArea)
        generatedTextArea.onwheel(e)
    },
    { passive: false },
  )
  ...
  document.getElementById('theme').addEventListener('click', () => {
    const theme = inverseSystemTheme()
  ...
  function updateHash(code, map) {
    try {
      ...
      if (finalLength >= kMaxURLDisplayChars)
        throw 'URL estimate too long'
